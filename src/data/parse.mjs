...
async function scrapeIFrameTable(url) {
    ...
    var tableRes = await page.$$eval("[data-code]",function(body){
        return (body[0].getAttribute("data-code"));
    });

    var $ = cheerio.load(tableRes);

    const tableData = [];

    await $('table tr').each((ind,row) => {
        const rowData = [];
        $('td',row).each((ind,cell) => {
            let text = $(cell).text();
...
const url = 'https://sites.google.com/view/goodluck2cg/%E5%B0%88%E5%B1%AC%E5%AF%B5%E7%89%A9?authuser=0';
...
async function Parse() {
    ...
    const ret = tableData.map((n) => {
        ...
        //     console.log(n);
        // }
        const attri = parseElementString(n[9]);

        //0 50 50 0→宠物的属性：依次为地、水、火、风

        content.push("編號" + (index + 1) + "=" + n[0] + "," + n[2], n[3], n[4], n[5], n[6], 20, jsonObject[n[1].replace("系","")],
            attri.地 * 10,
            attri.水 * 10,
            attri.火 * 10,
            attri.風 * 10,
        ...
        // console.log("編號" + (index + 1) + "=" + n[0] + "," + n[2], n[3], n[4], n[5], n[6], 20, jsonObject[n[1].replace("系","")],
        //     attri.地 * 10,
        //     attri.水 * 10,
        //     attri.火 * 10,
        //     attri.風 * 10,
    ...
    try{
        if (config.monster_path && "" != config.monster_path.trim()) {
            ...
            const utf16buffer = Buffer.from(`\ufeff` +  "[自製寵物]\r\n" + ret.join("\n"), 'utf16le');
    //     fs.writeFileSync('test-utf16le.txt', utf16buffer);
            fs.writeFileSync(config.monster_path, utf16buffer );
    ...
    const mappedpet2 = tableData.map(n => [null, n[0], null,
        n[2], n[3], n[4], n[5], n[6], 0.2
    ]);

    const custom = [[null, "小可愛優奈", null, "28", "28", "28", "28", "28", 0.2]];

    const mappedPetNames = new Set(mappedpet2.map(pet => pet[1]));
    const uniquePts2 = pts2.filter(pet => !mappedPetNames.has(pet[1]));
    let allpets = [...uniquePts2, ...custom, ...mappedpet2];
